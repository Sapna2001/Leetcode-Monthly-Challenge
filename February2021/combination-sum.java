...
class Solution {
    private void helper(List<List<Integer>> result ,int index, List<Integer> list, int[] candidates, int target) {
        if(target < 0) {
            return;
        } 
        ...
        for(int i = index; i < candidates.length ; i++) {
            ...
            helper(result, i, list, candidates, target - candidates[i]);
    ...
    public List<List<Integer>> combinationSum(int[] candidates, int target) {
        ...
        helper(result, 0, list, candidates, target);
        return result;
    }
}
